...
We can use structure induction, since the definition of formal deducibility is a *recursive* definition of the set of the proved theorems.

- The rule (Ref) is the **BASE** 
- The other 10 rules of formal deduction are the **RECURSION**

The **BASE CASE** of structural induction is to prove that $A \vdash A$, has a certain property

The **INDUCTIVE STEP** is to prove the other ten rules preserve the property

```ad-example
If we want to prove that all schemes of formal deducibility have a certain property, in the subcase $(\lor -)$ of the inductive step we suppose 
$$\Sigma, A \vdash C$$
$$\Sigma, B \vdash C \space(I.H.)$$  
have the property, then show 
$$\Sigma, A \lor B \vdash C$$
...
**Inductive Step:** We distinguish 10 cases, for each case, assume the starting theorems have the property, and prove the derived theorem has the property.

Case $(\to-)$
```

This proof captured the intuition that, since a proof for $\Sigma \vdash A$ involves only fintely many steps, we only need **fintely many formulas** from the premise set $\Sigma$ in the proof.

In the scheme of formal deducibility $\Sigma \vdash A$, the premise is a set of formulas, and the conclusion consists of one formula. 

```ad-note
When a number of schemes have the same premises, we stipulate to write $\Sigma \vdash \Sigma'$ for "For any $B \in \Sigma'$, we have that $\Sigma \vdash B$"
```


### Comparing $(\neg-)$ $(\neg+)$
- Both formalize the idea of "proof by contradiction", they are familar in shape but different in strength.
- $(\neg-)$ is stronger than $(\neg+)$, as the latter is proved by the former.
- Also, if $(\neg-)$ is replaced by $(\neg+)$ in the 11 rules, you can't prove $(\neg-)$
